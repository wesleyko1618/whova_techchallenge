...
class db_table:
    ...
    def insert(self, item):
        ...
        temp = []                                                               # add this to make it so syntax errors from non-sqlite substitutions don't occur anymore
        ...
        cursor.execute("INSERT INTO %s (%s) VALUES (?,?,?,?,?,?,?,?)" % (self.name, columns_query), (temp[0],temp[1],temp[2],temp[3],temp[4],temp[5],temp[6],temp[7]))
        cursor.close()
        self.db_conn.commit()
